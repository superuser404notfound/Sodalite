...
- **Navigation style (Apple TV)**: Settings → Tabs also chooses whether tvOS navigates from the bar across the top or from the collapsing sidebar that tvOS 26 uses in Apple TV, Music and Podcasts. The sidebar gives every screen the height the top bar reserves for itself, which the Live TV guide in particular spends on another row. The choice applies when you leave the screen, and it is stored on that Apple TV rather than synced, since iPhone and iPad are on the sidebar either way
